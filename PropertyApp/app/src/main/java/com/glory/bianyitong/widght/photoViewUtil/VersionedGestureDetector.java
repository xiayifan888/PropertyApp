...
public abstract class VersionedGestureDetector {
	...
	private static class CupcakeDetector extends VersionedGestureDetector {
		...
		float mLastTouchX;
		float mLastTouchY;
		private VelocityTracker mVelocityTracker;//用来追踪触摸事件（flinging事件和其他手势事件）的速率。用obtain()函数来获得类的实例
...
//					mIsDragging = FloatMath.sqrt((dx * dx) + (dy * dy)) >= mTouchSlop;
	...
	private static class FroyoDetector extends EclairDetector {
		...
		private final ScaleGestureDetector mDetector;//这个类是专门用来检测两个手指在屏幕上做缩放的手势用的
